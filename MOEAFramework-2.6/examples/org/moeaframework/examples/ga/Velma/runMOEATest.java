...
import gov.epa.velmasimulator.CalibratorConfiguration;
import gov.epa.velmasimulator.CalibratorConfigurationLoadFailedException;
import gov.epa.velmasimulator.CalibratorMoeaOutputLocation;
import gov.epa.velmasimulator.CalibratorObjectiveFunction;
import gov.epa.velmasimulator.CalibratorVelmaExclusive;
import gov.epa.velmasimulator.CalibratorVelmaParameter;
import gov.epa.velmasimulator.CalibratorVelmaParameterFollow;
import gov.epa.velmasimulator.ICalibratorConfigurationRecord;
import gov.epa.velmautils.Utils;

public class runMOEATest {
...
//                int divisions);
        ...
        while (!algorithm.isTerminated() && (algorithm.getNumberOfEvaluations() < Double.MAX_VALUE)) {
            ...
            for (int j = 0; j <refPop.size(); j++) {
            
            Solution indSoln = refPop.get(j);
            double[] Objs = indSoln.getObjectives();
...
//            System.out.println(refPop.get(0) + "," + refPop.get(1) );
