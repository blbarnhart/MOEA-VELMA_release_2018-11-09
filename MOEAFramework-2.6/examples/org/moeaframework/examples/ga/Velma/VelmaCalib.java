...
import gov.epa.velmamoeaapi.CalibratorVelmaParameterBase.VelmaFormat;
...
public class VelmaCalib extends AbstractProblem {
    ...
    protected CalibratorConfiguration calConfig;

    public VelmaCalib(CalibratorConfiguration calConfig) {
        super(getNumVars(calConfig),
                getNumObjs(calConfig));
        //                getNumConstraints());
    ...
    public Solution newSolution() {
        ...
        double[] minsMaxesDouble = getMinsMaxes(calConfig);
        int j = 0; 
        for (int i = 0; i < minsMaxesDouble.length/2; ++i){
    ...
    public void evaluate(Solution solution) {
        ...
        if (simConfigXmlFullName == null) {
            System.out.println("FAIL: CalibratorConfiguration file doesn't specify a VELMA_SOURCE record.");
        }
        ...
        for (int i = 0; i < params.size(); ++i) {
            ...
            if (velmaRecord.getDefaultValueItemCount() == 1) {
                kvPropertyOverrides.put(velmaRecord.getVelmaKeyName(), velmaRecord.getFormattedText(x[count]));
                System.out.println(velmaRecord.getFormattedText(x[count]));
                count = count + 1; 
            }
            else {
                ...
                for (int ivary : velmaRecord.getVaryingItemIndices()){
                    tempList2[ivary] = velmaRecord.getFormattedText(x[count]);
                    count = count + 1; 
                }
        ...
        if (vcr.isAllLoopsDone() != true) {
            ...
            for (int i = 0; i < objRecs.size(); ++i) {
                ...
                allObjectivesToInclude.put(objRecord.getObjectiveKeyName(), 
                        getObjectiveCalculation(calConfig, vcr, 
     ...
     */
    public static int getNumObjs(CalibratorConfiguration calConfig) { 
    ...
    public static int getNumVars(CalibratorConfiguration calConfig) { 
        ...
        for (int i = 0; i < params.size(); ++i) {
            ...
            for (int ivary : velmaRecord.getVaryingItemIndices()) {
                varsNumber.add(String.valueOf(velmaRecord.getVaryingMinValue(ivary)));
            }
        }
    ...
    public double[] getMinsMaxes(CalibratorConfiguration calConfig) { 
        // This gets the mins and maxes from the VELMA_PARAMETERS from the Calibrator
        ...
        for (int i = 0; i < params.size(); ++i) {
            ...
            for (int ivary : velmaRecord.getVaryingItemIndices()) {
                ...
                paramsMinMax.add(String.valueOf(velmaRecord.getVaryingMinValue(ivary)));
                paramsMinMax.add(String.valueOf(velmaRecord.getVaryingMaxValue(ivary)));
    ...
    public double getObjectiveCalculation(
            CalibratorConfiguration calConfig,
    ...
    public static double getListRMSE(List<Double> obsData, List<Double> simData) {
        ...
        //break from method if no obs data available. 
//        if (obsDataD.length == 0) { 
//            return  
//        }
