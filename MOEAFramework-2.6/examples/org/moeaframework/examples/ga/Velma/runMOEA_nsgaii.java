...
public class runMOEA_nsgaii {
    ...
    public static void main(String[] args) {
        ...
        final int populationSize = (int) Long.parseLong(args[2]);
        final int numberOfProcessorsToUse = (int) Long.parseLong(args[3]);
        //final int populationSize = 16;
        //final int populationSize = Runtime.getRuntime().availableProcessors();

        System.out.println("Number of cores used in this run is" + numberOfProcessorsToUse);
        System.out.println("Number of individuals in this run is" + populationSize);

        /////////////////////////////////////////////
        //Set up distributed VelmaProblem 
        /////////////////////////////////////////////
        ExecutorService executor = Executors.newFixedThreadPool(numberOfProcessorsToUse);
        
        //Insert problem name into this -- VelmaCalib or VelmaCalibWithConstraint are two examples. 
        final Problem problem = new DistributedProblem(new VelmaCalib(calConfig), executor);
        ...
        while (!algorithm.isTerminated() && (algorithm.getNumberOfEvaluations() < Double.MAX_VALUE)) {
            ...
            TreeMap<String, String> paramsMap1 = runMOEA_nsgaii.getParamsMap(calConfig, currPop.get(0));
            TreeMap<String, String> followersMap1 = runMOEA_nsgaii.getFollowersMap(calConfig, currPop.get(0));
            TreeMap<String, String> exclusivesMap1 = runMOEA_nsgaii.getExclusivesMap(calConfig, currPop.get(0));
            ...
            try {
                ...
                for (int s = 0; s < currPop.size(); s++) {
                    ...
                    TreeMap<String, String> paramsMap = runMOEA_nsgaii.getParamsMap(calConfig, indSoln);
                    TreeMap<String, String> followersMap = runMOEA_nsgaii.getFollowersMap(calConfig, indSoln);
                    TreeMap<String, String> exclusivesMap = runMOEA_nsgaii.getExclusivesMap(calConfig, indSoln);

                    fw.write("objectives,");
                    //Write all objective values
                    for (int i = 0; i < indSoln.getNumberOfObjectives(); i++) {
                        //                        CalibratorObjectiveFunction objGroupRecord = (CalibratorObjectiveFunction)objGroupRecs.get(i);                      
                        double objectiveValue = indSoln.getObjective(i);
                        //                        if (objGroupRecord.getObjectiveChoice().equals("NSE")) {
    ...
    public static TreeMap<String, String> getFollowersMap(CalibratorConfiguration calConfig, Solution indSoln) { 
        ...
        for (int i = 0; i < followers.size(); ++i) {
            CalibratorVelmaParameterFollow followerRecord = (CalibratorVelmaParameterFollow)followers.get(i);           
            followersMap.put(followerRecord.getFollowerVelmaKeyName(), 
                    paramsMap.get(followerRecord.getLeaderVelmaKeyName()));
        }
